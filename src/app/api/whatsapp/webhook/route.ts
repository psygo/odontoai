...
import { db } from "@/db";
import { clinics, conversations, customers, messages } from "@/db/schema";
import { respondToCustomerMessage } from "@/lib/ai/agent";
...
export async function POST(req: NextRequest) {
  ...
  // take long enough to look like one — without this, a single customer
  // message can produce two independent (differently-worded) AI replies.
  // This is just a fast-path optimization (skip customer/conversation work on
  // an obvious duplicate); the real dedup guarantee is the atomic insert
  // inside respondToCustomerMessage. So a transient DB error here should fail
  ...
  let customer = await db.query.customers.findFirst({
    where: and(eq(customers.clinicId, clinic.id), eq(customers.phone, phone)),
  });
  if (!customer) {
    [customer] = await db
      .insert(customers)
  ...
  if (!conversation) {
    [conversation] = await db
      ...
      .values({ clinicId: clinic.id, customerId: customer.id })
      .returning();
  }
  ...
  const replyText = await respondToCustomerMessage({
    clinicId: clinic.id,
